...
export class UiFactory {
  ...
  square() {
    return [
      ...
      [{ text: "Город", callback_data: "go:City" }],
  ...
  city() {
    return [
      ...
      [{ text: "Доска почета", callback_data: "go:CityBoard" }],
      [{ text: "Назад в город", callback_data: "go:Square" }],
    ];
  }
  shopHub() {
  ...
  bar(user, now = Date.now()) {
    ...
    kb.push([{ text: "Показать задания бара", callback_data: "bar:tasks" }]);
  ...
  barTasks(user) {
    ...
    if (!tasks.length) {
      ...
      kb.push([{ text: "Назад", callback_data: "go:Bar" }]);
      return kb;
    }
...
// ---------- Работа ----------
  workV2(user, options = {}) {
    ...
    if (active) {
      if (ready) {
        ...
        kb.push([{ text: "Отменить смену", callback_data: "work:cancel" }]);
      } else {
    ...
    kb.push([{ text: "Назад в город", callback_data: "go:Square" }]);
    return kb;
  }
  ...
  studyIdle(effectsText) {
    return [
      ...
      [{ text: "Назад к прогрессу", callback_data: "go:Progress" }],
    ];
  }
  ...
  home(user, opts = {}) {
    ...
    if (!user.rest.active) {
      ...
      kb.push([{ text: `Остановить отдых (x${mult})`, callback_data: "rest:stop" }]);
    }
    ...
    kb.push([{ text: `Текущая кровать: ${currentTitle} (x${mult})`, callback_data: "noop" }]);
    ...
    kb.push([{ text: "Назад", callback_data: `go:${back}` }]);
    return kb;
  }
  ...
  shop(opts = {}) {
    ...
    const backText =
      backTo === "Work"  ? "Назад к сменам" :
      backTo === "Study" ? "Назад к учебе" :
      backTo === "Gym"   ? "Назад в зал" :
                           "Назад";
  ...
  casinoMenu() {
    ...
    rows.push([
      ...
      { text: "Назад", callback_data: "go:Earn" }
    ]);
    return rows;
  }
  ...
  gym(user, now = Date.now(), ffCost = null) {
    ...
    return [
      [{
        text: `Начать тренировку ($${costMoney}, ${costEnergy}⚡, ~${mins} мин)`,
  ...
  upgrades(user) {
    ...
    rows.push([{ text: "Назад", callback_data: "go:Progress" }]);
    return rows;
  }
  ...
  cityTopStrong() {
    return [
      [{ text: "Назад", callback_data: "go:CityBoard" }],
    ];
  }
