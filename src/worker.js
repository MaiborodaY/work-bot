...
if (cb.game_short_name && cb.game_short_name === (env.TD_GAME_SHORT_NAME || "")) {
  const payload = cb.inline_message_id
    ? { user_id: cb.from.id, inline_message_id: cb.inline_message_id }
    : { user_id: cb.from.id, chat_id: cb.message.chat.id, message_id: cb.message.message_id };
  ...
  const baseTd = String(env.TD_PAGES_URL || env.PAGES_URL || "").replace(/\/+$/,'');
  const openUrlTd = `${baseTd}/?p=${encodeURIComponent(p)}`;
  try { await bot.answerCallbackUrl(cb.id, openUrlTd); } catch {}
  return new Response("ok");
}
