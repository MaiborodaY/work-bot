...
export class Locations {
  ...
  async _renderSquareRoute(user, { header = "", lang = "ru", onboardingStage = "" } = {}) {
  ...
  async _renderCityBoardRoute(user, { header = "", lang = "ru" } = {}) {
    await this.media.show({
      sourceMsg: this._sourceMsg,
      place: Routes.CITY_BOARD,
    ...
    this._sourceMsg = null;
    this._route = Routes.CITY_BOARD;
  }

  async _renderStudyRoute(user, { header = "", lang = "ru" } = {}) {
  ...
  async _renderHomeRoute(user, { lang = "ru" } = {}) {
    ...
    this._sourceMsg = null;
    this._route = Routes.HOME;
  }

  async _renderShopRoute(user, { header = "", lang = "ru" } = {}) {
    ...
    this._sourceMsg = null;
    this._route = Routes.SHOP;
  }

  async _renderCasinoRoute(user, { lang = "ru" } = {}) {
    ...
    this._sourceMsg = null;
    this._route = Routes.CASINO;
  }

  async _renderBarRoute(user, { lang = "ru" } = {}) {
    ...
    this._sourceMsg = null;
    this._route = Routes.BAR;
  }

  async _renderGymRoute(user, { introText = null, lang = "ru", onboardingStage = "" } = {}) {
    let ffCost = null;
    try {
    ...
    try {
      ...
      const backText =
        backToGym === Routes.WORK ? this._t(user, "loc.gym.back_work") :
        backToGym === Routes.STUDY ? this._t(user, "loc.gym.back_study") :
    ...
    this._sourceMsg = null;
    this._route = Routes.GYM;
  }

  async _renderUpgradesRoute(user, { lang = "ru" } = {}) {
    ...
    this._sourceMsg = null;
    this._route = Routes.UPGRADES;
  }

  async _renderBarTasksRoute(user, { lang = "ru" } = {}) {
    ...
    this._sourceMsg = null;
    this._route = Routes.BAR_TASKS;
  }

  async _renderBusinessRoute(user, { header = "", lang = "ru", route = Routes.BUSINESS } = {}) {
    const bizRouteMeta = {
      Biz_shawarma: {
        id: "shawarma",
    ...
    const renderBusinessCard = async (B, options = {}) => {
      ...
      this._sourceMsg = null;
      this._route = opts.routeName || Routes.BUSINESS;
    };
  ...
  async _renderFallbackSquareRoute(user, { header = "", lang = "ru" } = {}) {
