...
if (changed) {
  ...
  /** Проверка по новой схеме: jobs.active[0] завершена и ещё не уведомлена. */
  _hasReadyWork(u, now) {
    const inst = u?.jobs?.active?.[0];
...
/** Проверка study*/
  ...
  /** Собираем готовых пользователей либо через listAll, либо сканируя KV. */
  ...
  /** Список префиксов к перебору — даже если kvPrefix задан, пробуем и типовые. */
  _candidatePrefixes() {
    const list = [];
  ...
  /** Скан KV по префиксу и фильтр готовых. */
